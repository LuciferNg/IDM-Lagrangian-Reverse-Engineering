...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    if (flag1 == _TRUE_) {
      ...
      else if ((strstr(string1,"IDM") != NULL) || (strstr(string1,"idm") != NULL)) {
        pba->fluid_equation_of_state = IDM;
      }
      else {
    ...
    if (pba->fluid_equation_of_state == IDM) {
      /** 8.a.2.4) Equation of state of the fluid in 'IDM' case */
      class_read_double("eps_idm",pba->eps_idm);
      class_read_double("zc_idm",pba->zc_idm);
      pba->w0_fld = -1.0;
      pba->wa_fld = 0.0;
    }
  }
