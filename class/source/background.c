...
int background_w_fld(
  ...
  case EDE:
    ...
    *w_fld = - dOmega_ede_over_da*a/Omega_ede/3./(1.-Omega_ede)+a_eq/3./(a+a_eq);
    break;
  case IDM:
    /* IDM: f(z) = 1 - eps*(z/zc)*exp(-z/zc)
       w(z) = -1 + (1+z)/(3*zc) * f'(z)/f(z) */
    z = 1./a - 1.;
    x = z / pba->zc_idm;
    if (isfinite(x) && fabs(x) < 50.) {
      f_idm = 1. - pba->eps_idm * x * exp(-x);
      fp_idm = -pba->eps_idm / pba->zc_idm * exp(-x) * (1. - x);
      if (fabs(f_idm) > 1e-30)
        *w_fld = -1. + (1.+z)/(3.*pba->zc_idm) * fp_idm / f_idm;
      else
        *w_fld = -1.;
    } else {
      *w_fld = -1.;
  ...
  case IDM:
    {
      double da = 1e-5;
      double a_hi = a + da, a_lo = fmax(a - da, 1e-10);
      double z_hi = 1./a_hi - 1., z_lo = 1./a_lo - 1.;
      double x_hi = z_hi/pba->zc_idm, x_lo = z_lo/pba->zc_idm;
      double fh = 1.-pba->eps_idm*x_hi*exp(-x_hi);
      double fl = 1.-pba->eps_idm*x_lo*exp(-x_lo);
      double wh = -1.+(1.+z_hi)/(3.*pba->zc_idm)*(-pba->eps_idm/pba->zc_idm*exp(-x_hi)*(1.-x_hi))/fh;
      double wl = -1.+(1.+z_lo)/(3.*pba->zc_idm)*(-pba->eps_idm/pba->zc_idm*exp(-x_lo)*(1.-x_lo))/fl;
      *dw_over_da_fld = (wh-wl)/(a_hi-a_lo);
    }
    break;
  }
  ...
  case IDM:
    {
      double zz = 1./a - 1.;
      double xx = zz / pba->zc_idm;
      double ff = 1. - pba->eps_idm * xx * exp(-xx);
      *integral_fld = (ff > 0.) ? log(ff) : 0.;
    }
    break;
  }
